...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'enquiry.list' });

  return buildMetadata({
    locale,
    title: t('title'),
    description: t('description'),
    path: routes.enquiries(locale),
  });
}
 ...
 */
export default async function ConsumerEnquiriesPage({ params }: Props) {
  const { locale } = await params;
