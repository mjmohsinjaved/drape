...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'enquiry.new' });

  return buildMetadata({
    locale,
    title: t('title'),
    description: t('description'),
    path: routes.enquiryNew(locale),
  });
}
 ...
 */
export default async function ConsumerEnquiriesNewPage({ params }: Props) {
  const { locale } = await params;
