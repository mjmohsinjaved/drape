...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'admin.settingsPolicy' });

  return buildMetadata({
    locale,
    title: t('title'),
    description: t('description'),
    path: routes.admin.settingsPolicy(locale),
  });
}
 ...
 */
export default async function AdminSettingsPolicyPage({ params }: Props) {
  const { locale } = await params;
