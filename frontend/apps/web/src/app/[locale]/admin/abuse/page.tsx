...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: 'admin.abuse' });

  return buildMetadata({
    locale,
    title: t('title'),
    description: t('description'),
    path: routes.admin.abuse(locale),
  });
}

/**
 * TODO(W5): replace `PagePlaceholder` with the feature component. The route, the
 ...
 */
export default async function AdminAbusePage({ params }: Props) {
  const { locale } = await params;
