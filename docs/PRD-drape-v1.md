...
- **S-8** Authentication is by email and password alone. There is no second factor for either role — no TOTP, no emailed or texted sign-in PIN, no recovery codes. (Superseded: this requirement previously made 2FA mandatory for Admin and optional for Consumer.)
...
- **C-7** Account settings: update profile, change password, manage notification preferences.
...
- Argon2 password hashing; email and password are the only credentials (S-8)
...
users             id, role, email, email_verified_at, password_hash, name,
                  phone, phone_verified_at, status,
